...
const click = (0, tool_1.defineTool)({
  ...
  handle: async (context, params) => {
    ...
    const code = [`// Click ${params.element}`, `await page.${await (0, context_1.generateLocator)(locator)}.click();`];
    return {
      code,
...
const drag = (0, tool_1.defineTool)({
  ...
  handle: async (context, params) => {
    ...
    const code = [
      ...
      `await page.${await (0, context_1.generateLocator)(startLocator)}.dragTo(page.${await (0,
      context_1.generateLocator)(endLocator)});`,
    ];
    return {
      code,
...
const type = (0, tool_1.defineTool)({
  ...
  handle: async (context, params) => {
    ...
    if (params.slowly) {
      ...
      code.push(
        `await page.${await (0, context_1.generateLocator)(locator)}.pressSequentially(${javascript.quote(
          params.text
        )});`
      );
...
const screenshotSchema = zod_1.z
  .object({
    raw: zod_1.z
      .boolean()
      .optional()
...
const screenshot = (0, tool_1.defineTool)({
  ...
  handle: async (context, params) => {
    ...
    const fileName =
      path_1.default.join(os_1.default.tmpdir(), (0, utils_1.sanitizeForFilePath)(`page-${new Date().toISOString()}`)) +
      `.${fileType}`;
    ...
    if (locator)
      code.push(
        `await page.${await (0, context_1.generateLocator)(locator)}.screenshot(${javascript.formatObject(options)});`
      );
    else code.push(`await page.screenshot(${javascript.formatObject(options)});`);

    const action = async () => {
      if (locator) await locator.screenshot(options);
      else await tab.page.screenshot(options);
...
exports.default = [snapshot, click, drag, hover, type, selectOption, screenshot];
